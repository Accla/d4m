...
/**
 * This class provides a way of encoding a path into a 
 * binary tree into a string.
 * 
 * The encoding is constructed in a such a way that
 * siblings should come togther when the encodings
 * are sorted.
 * 
 *
 */
...
public class EncodedBinaryTreePath {
	...
	static Logger log = Logger.getLogger(EncodedBinaryTreePath.class.getName());
	
	public EncodedBinaryTreePath(){
		len = 0;
		path = new BigInteger("0");
	}
	...
	public static void main(String[] args) {
		for(int len = 0 ; len < 22; len++){
			...
			for(int i = 0; i < 1 << len; i++){
				...
				//System.out.println(ebtp.getEncoding()+" "+ebtp);
