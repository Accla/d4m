% Plot data taken from Jaccard experiments and create a table.
doplot = false;

Hybrid = false; % use non-hybrid data Jaccard
...
for rowmati = 1:size(rowmat,1)
    ...
    if (strcmp(engine,'graphulo') || strcmp(engine,'dense'))
        ...
        thisHybrid = Val(str2num(Arow(:,'Hybrid,')));
        if isempty(thisHybrid)
            thisHybrid = false;
        end
        if thisHybrid ~= Hybrid
        	continue;
        end
    end
    ...
    if isempty(tidx)
        ...
        t.jaccardnumpp(newlen) = numpp;
        t.adjnnz(newlen) = Val(str2num(Atname(:,'nnz,')));
        t.jaccardnnz(newlen) = Val(str2num(Atname(:,['jaccardNnz,'])));
    end
...
for engine = {{'graphulo', 'Graphulo'}, {'d4m', 'D4M'}}
	...
	for ntidx = 1:numel(d.(en).nt)
        nt = d.(en).nt(ntidx);
        if strcmp(en,'d4m') && Hybrid
            legendarr{legendarri} = [ens{2} ' ' num2str(nt) ' Tablet Hybrid'];
          else
            legendarr{legendarri} = [ens{2} ' ' num2str(nt) ' Tablet'];
          end 
        legendarri = legendarri + 1;
...
t.jaccardnnz = t.jaccardnnz(sortidx);
t.jaccardnumpp = t.jaccardnumpp(sortidx);

dincls = [1,2];

a = [minscale:maxscale; t.adjnnz; t.jaccardnnz; t.jaccardnumpp; d.graphulo.time{1}; d.graphulo.time{2};  d.d4m.time{1}; d.d4m.time{2}];
for di = dincls
    a = [a; d.dense.time{di}];
end
...
b = [b(1:4,:); arrayfun(@(s) [num2str(s,'%.1f') 'x'], t.jaccardnumpp ./ t.jaccardnnz, 'UniformOutput', false); b(5:end,:)];
...
inp = struct();
inp.data = b;
inp.tableRowLabels = {'SCALE','$\operatorname{nnz}(\matr{A})$','$\operatorname{nnz}(Jaccard(\matr{A}))$','Total Partial Products','Graphulo Overhead','D4M Time 1 Tablet (s)','D4M Time 2 Tablets (s)','Graphulo Time 1 Tablet (s)','Graphulo Time 2 Tablets (s)'};
for di = dincls
    inp.tableRowLabels = [inp.tableRowLabels {['Dense ' num2str(di)]}];
end
inp.tableCaption = 'Comparison of Writes and Runtimes for Jaccard algorithm'; %Figure~\ref{fKTrussAdjTime}
inp.tableLabel = 'tJaccardTable';
inp.dataFormat = {'%s'};
...
lat{4} = '\adjustbox{angle=30,lap={1.75em}{\width-3.8em},raise=-0.75em,scale=0.95}{SCALE} & $\operatorname{nnz}(\matr{A})$ & \begin{tabular}{@{}c@{}}$\operatorname{nnz}($ \\ $\jaccard{}(\matr{A}))$ \end{tabular} & \begin{tabular}{@{}c@{}} Partial \\ Products \end{tabular} & \begin{tabular}{@{}c@{}} Graphulo \\ Overhead \end{tabular} & \begin{tabular}{@{}c@{}}Graphulo \\1 Tablet \end{tabular} & \begin{tabular}{@{}c@{}}Graphulo \\2 Tablets \end{tabular} & \begin{tabular}{@{}c@{}}D4M \\1 Tablet \end{tabular} & \begin{tabular}{@{}c@{}}D4M \\2 Tablets \end{tabular}  ';
for di = dincls
