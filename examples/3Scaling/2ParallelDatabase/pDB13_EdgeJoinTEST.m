...
E1 = Abs0(TedgeIt1(:,colRange1));           % Start first query iterator.
E1outDeg = Assoc('','','');
while nnz(E1)
  E1 = Abs0(Tedge(Row(E1),:));              % Get edges containing these in vertices.
  E1outDeg = E1outDeg + sum(E1(:,StartsWith('Out|,')),1);    % Compute out degree.
  E1 = Abs0(TedgeIt1());                    % Get next query.
end

E2 = Abs0(TedgeIt2(:,colRange2));           % Start first query iterator.
E2outDeg = Assoc('','','');
while nnz(E2)
  E2 = Abs0(Tedge(Row(E2),:));              % Get edges containing these in vertices.
  E2outDeg = E2outDeg + sum(E2(:,StartsWith('Out|,')),1);    % Compute out degree.
  E2 = Abs0(TedgeIt2());                    % Get next query.
end
