%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Benchmark associative array multiply performance.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:numel(n);
  ...
  tic;
    C = A*B;                                % Perform associative array multiply.
  assoc_time(i) = toc;

  assoc_flops(i)  = full(2*sum(sum(Adj(C))));    % Compute flops to be credited.
  [ii jj vv] = find(C);
  assoc_gbytes(i) = assoc_gbytes(i) + (numel(ii) + numel(jj) + 8.*m(i)) ./ 1e9;
...
AassocResults  = Assoc(r,'N,',n.') + Assoc(r,'M,',m.') + Assoc(r,'time,',assoc_time.') +  Assoc(r,'GByte,',assoc_gbytes.') + Assoc(r,'GFlop,', assoc_gflops.') + Assoc(r,'MemFrac,',(assoc_gbytes./MaxGB).') + Assoc(r,'ProcFrac,', (assoc_gflops./MaxGF).');

displayFull(AassocResults);                 % Show results in tabular form.
