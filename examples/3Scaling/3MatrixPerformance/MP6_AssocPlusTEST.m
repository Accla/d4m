...
n = 2.^[6 6 7 8 9 10 11 12 13 14];    % Set matrix sizes to test.
...
%  tic;
%    C1 = plus1(A,B);                                 % Perform associative array addition.
%  toc
  tic;
    C = A + B;                                   % Perform associative array addition.
  assoc_time(i) = toc;
%  C1 - C
%  if nnz(C1 -C)
%    keyboard
%  end
  ...
  assoc_flops(i)  = full(sum(sum(Adj(C))));          % Compute flops to be credited.
  [ii jj vv] = find(C);
  assoc_gbytes(i) = assoc_gbytes(i) + (numel(ii) + numel(jj) + 8.*m(i)) ./ 1e9;
...
AassocPlusResults  = Assoc(r,'N,',n.') + Assoc(r,'M,',m.') + Assoc(r,'time,',assoc_time.') +  Assoc(r,'GByte,',assoc_gbytes.') + Assoc(r,'GFlop,', assoc_gflops.') + Assoc(r,'MemFrac,',(assoc_gbytes./MaxGB).') + Assoc(r,'ProcFrac,', (assoc_gflops./MaxGF).');

displayFull(AassocPlusResults);                      % Show results in tabular form.

save('data/MP6_AssocPlus.mat','AassocPlusResults');   % Save output for comparison.

semilogx(full(Adj(AassocPlusResults(2:numel(n),'MemFrac,'))),full(Adj(AassocPlusResults(2:numel(n),'ProcFrac,'))));
